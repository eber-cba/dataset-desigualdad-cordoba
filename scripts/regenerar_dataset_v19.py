...
if barrios_inventados:
    ...
    audit_report.append(f"**⚠️ ALERTA:** Se detectaron {len(barrios_inventados)} barrios sin validación oficial. Fueron excluidos paramétricamente.")
else:
...
if has_geo:
    ...
    if lat_inv.sum() > 0 or lon_inv.sum() > 0:
        ...
        audit_report.append("- ✅ **Bounding Box:** Coordenadas paramétricamente válidas en Provincia de Córdoba.")
...
if hog_pob > 0: df['hogares'] = np.where(df['hogares'] > df['poblacion'], df['poblacion'], df['hogares'])
...
audit_report.append(f"- Se detectaron **{(df['outlier_flag'] == -1).sum()} barrios extremadamente atípicos** (High End Extrema o Subdesarrollo Crítico).")
df[df['outlier_flag'] == -1][['barrio'] + cols_clustering].to_csv("data/processed/outlier_barrios_v19.csv", index=False)
...
if H_stat > 0.8: interp_H = "Clusterabilidad Fuerte. Las entidades no están dispersas al azar, hay una tremenda tendencia al agrupamiento natural."
elif H_stat > 0.7: interp_H = "Clusterabilidad Aceptable (Estructura de enjambre real detectada)."
else: interp_H = "Clusterabilidad Débil (La estructura es casi un ruido blanco, difícil particionar)."

audit_report.append(f"- **Interpretación Metodológica:** {interp_H}")
audit_report.append("</div>")
pd.DataFrame([{'Hopkins_Statistic': H_stat, 'Interpretation': interp_H}]).to_csv("data/processed/hopkins_test_v19.csv", index=False)
...
if has_geo:
    ...
    audit_report.append(f"- Encontramos **{n_db_clusters} grandes manchas territoriales (Clusters DBSCAN)** aislando componentes geográficos puros.")
    audit_report.append(f"- Ruido / Barrios Aislados (Geográficamente disociados): {n_noise}")
    audit_report.append("- **Interpretación Territorial:** Mientras K-Means agrupa matemáticamente por estrato sociodemográfico transversal (cortando la ciudad en capas invisibles), el modelo DBSCAN agrupa por *Adyacencia Pura*. En Urbanismo, K-Means expone la segregación social sin importar donde vivas, mientras DBSCAN dictamina las fallas de transporte y cohesión física.")
    
    df_valid[['barrio', 'centroide_lat', 'centroide_lon', 'cluster_barrio', 'dbscan_cluster']].to_csv("data/processed/dbscan_clusters_v19.csv", index=False)
audit_report.append("</div>\n")
...
audit_report.append(f"- **Estabilidad K-Means (ARI, 50-splits):** {np.mean(ari_scores):.4f} (Altamente Estable)")
...
if has_geo:
    ...
    try:
        ...
        audit_report.append(f"- **Moran's I (Infraestructura):** {mi.I:.4f} (p-value: {mi.p_sim}). Demuestra contagio y aglomeración territorial fuerte en vez de ruido aleatorio.")
...
for i in range(best_k_final):
    ...
    if rank_infra == best_k_final - 1: perfil = "Núcleo Urbano Consolidado"
    elif rank_nbi == best_k_final - 1: perfil = "Periferia Vulnerable"
    elif rank_nbi >= (best_k_final // 2): perfil = "Suburbio Popular Consolidado"
    else: perfil = "Anillo Denso Residencial"
...
if has_geo:
    ...
    m = folium.Map(location=[-31.42, -64.18], zoom_start=12, tiles="CartoDB dark_matter")
    # Colores semanticos fijos map (ej 5 tipos max)
    colormap = {'Núcleo Urbano Consolidado': '#0ea5e9', 
    ...
    audit_report.append("- El entorno vectorizó `mapa_interactivo_clusters_v19.html`, conteniendo un motor interactivo (Web/DOM) para navegación por zoom profunda del conurbano analizado.")
...
try:
    ...
    core_num = [c for c in df.select_dtypes(include=[np.number]).columns if c not in ['pca_x', 'pca_y', 'outlier_flag']]
...
with open("final_data_science_audit_v19.md", "w", encoding='utf-8') as f: f.write("\n".join(audit_report))
