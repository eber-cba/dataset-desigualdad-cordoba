...
gdf = gpd.GeoDataFrame(df_geo, geometry=[Point(xy) for xy in zip(df_geo['centroide_lon'], df_geo['centroide_lat'])], crs="EPSG:4326")
...
for desc in gdf['cluster_descripcion'].unique():
    ...
    subset.plot(ax=ax, markersize=50, color=color, label=desc, alpha=0.85, edgecolor='black', linewidth=0.5)

ax.legend(title="Tipología Urbana Final", loc='best')
plt.title("Zonificación Socio-Urbana de Córdoba (Modelado K-Means)", fontsize=14, fontweight='bold')
...
sns.scatterplot(
    ...
    palette=colormap, s=100, alpha=0.8, edgecolor='black', ax=ax
)
plt.title(f"PCA Separación de Clusters (Varianza Explicada: {var_expl:.1f}%)", fontweight='bold')
